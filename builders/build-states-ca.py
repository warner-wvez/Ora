...
def main():
    ...
    for r in rows:
        ...
        if (parts[4] if len(parts)>4 else '1')!='1': inactive+=1   # kept: the pin still marks a camera location
        try: lngf,latf=float(lng),float(lat)
        except ValueError: continue
        iu=img_from_page(page)   # None for the few non-standard page URLs -> location-only pin
        feats.append({'type':'Feature','geometry':{'type':'Point','coordinates':[lngf,latf]},
            'properties':{'name':re.sub(r'\s*\([A-Z]\d+\)\s*$','',desc).strip() or 'Camera','kind':'live',
                          'directions':[{'snapshot':iu or None,'video':None,'label':''}],'roadway':'','county':''}})
    ...
    print(f'California: {len(feats)} cameras ({inactive} inactive but kept)')
